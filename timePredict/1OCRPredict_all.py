...
class OCRPredictionTool:
    def __init__(self, languages=['en', 'ch_sim'], measure_load_time=True):
        ...
        self.model_path = "model/ocr_final.pkl"  # 修改为合并后的模型路径
    ...
    def train_model(self):
        ...
        # 计算新的目标变量：execution_time + load_time
        df['total_time'] = df['execution_time'] + df['load_time']
            
        # 训练合并后的总时间预测模型
        self.model_trainer = ModelTrainer(
            feature_columns=self.features,
            target_column="total_time",  # 使用新的目标变量
            model_path=self.model_path
        )
    ...
    def predict_total_time(self, width: int, height: int, dpi: int = 72, 
                          ...
                          **kwargs) -> float:
        """预测总执行时间（execution_time + load_time）"""
...
def main():
    ...
    # 执行预测
    print("Predicting total time (execution_time + load_time)...")
    try:
        # 使用指定的图片
        ...
        if features:
            ...
            if predicted_total_time is not None:
                print(f"Predicted Total Time (execution_time + load_time): {predicted_total_time} seconds")
        else:
            print("无法计算图像特征")
    except Exception as e:
